...
def cached_tifffile(format: Union[AbstractFormat, CachedDataPath]) -> tifffile.TiffFile:
...
class TifffileChecker(SignatureChecker):
    ...
    def match(cls, pathlike: CachedDataPath) -> bool:
        ...
        if not (
            len(buf) > 2 and (buf[0] == buf[1] == 0x49 or buf[0] == buf[1] == 0x4D)
        ):
            return False
...
class TifffileParser(AbstractParser):
    ...
    @staticmethod
    def parse_acquisition_date(date: Union[datetime, str]) -> Union[datetime, None]:
        """
        ...
        if isinstance(date, datetime):
            return date
        if not isinstance(date, str) or (len(date) != 19 or date[16] != ":"):
            return None
    ...
    def parse_raw_metadata(self) -> MetadataStore:
        ...
        skipped_tags = (
            ...
            34675,
            50839,
        ) + tuple(range(65420, 65459))

        for tag in baseline.tags:
            if tag.code not in skipped_tags and type(tag.value) not in (
                bytes,
                np.ndarray,
            ):
