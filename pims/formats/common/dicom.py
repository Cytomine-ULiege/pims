...
class DicomParser(AbstractParser):
    ...
    def parse_known_metadata(self) -> ImageMetadata:
        ...
        imd.physical_size_z = self.parse_physical_size(ds.get("SliceSpacing"))
    ...
    def parse_raw_metadata(self) -> MetadataStore:
        ...
        for data_element in ds:
            if (
                type(data_element.value) in (bytes, bytearray)
    ...
    * For multi-frame, multi-sample data (frames, rows, columns, planes)
    ...
    def read_window(self, region, out_width, out_height, c=None, z=None, t=None):
        ...
        return self._pixel_array(
            np.s_[region.left : region.right], np.s_[region.top : region.bottom], c, z
        )
