...
# pylint: disable=invalid-name,function-redefined,too-many-statements,unused-argument
...
@router.post(
    "/image/{filepath:path}/normalized-tile{extension:path}", tags=norm_tile_tags
)
async def show_tile_with_body(
    request: Request,
    response: Response,
...
def zoom_query_parameter(zoom: int = PathParam(...)):
    return TargetZoom(__root__=zoom).dict()["__root__"]


def level_query_parameter(level: int = PathParam(...)):
    return TargetLevel(__root__=level).dict()["__root__"]


def ti_query_parameter(ti: int = PathParam(...)):
    return TileIndex(__root__=ti).dict()["__root__"]


@router.get(
    "/image/{filepath:path}/tile/zoom/{zoom:int}/ti/{ti:int}{extension:path}",
    tags=tile_tags,
)
...
@router.get(
    "/image/{filepath:path}/tile/level/{level:int}/ti/{ti:int}{extension:path}",
    tags=tile_tags,
)
