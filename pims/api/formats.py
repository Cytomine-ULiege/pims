...
class Format(BaseModel):
    id: FormatId
    name: str = Field(..., description="Readable format name", example="Hamamatsu VMS")
    ...
    readable: bool = Field(
        ...,
        description=(
            "Whether the format is natively readable by PIMS or not. "
    ...
    importable: bool = Field(
        ...,
        description=(
            "Whether the format is importable by an upload in PIMS or not. "
...
def list_formats():
    ...
    return response_list(
        sorted(correct_list, key=lambda x: x.id.__root__, reverse=False)
    )
