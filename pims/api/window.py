...
# pylint: disable=invalid-name,too-many-branches,too-many-statements,unused-argument
...
def _show_window(
    ...
    req_size = get_window_output_dimensions(
        in_image,
        region,
        height,
        width,
    ...
    if (
        annotations
        and annotation_style
        and not isinstance(annotations, ParsedAnnotations)
    ):
    ...
    if (
        annotations
        and annotation_style
        and annotation_style["mode"] == AnnotationStyleMode.MASK
    ):
        window = MaskResponse(
            in_image,
            annotations,
