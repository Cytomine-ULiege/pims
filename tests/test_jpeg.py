...
from pims.files.image import Image as PimsImage
...
def get_image(path, filename):
    ...
    if not os.path.exists(os.path.join(path, "processed/histogram")):
        ...
        try:
            ...
            fi.original = PimsImage(fi.original_path, format=format)
...
def test_jpeg_norm_tile(client, image_path_jpeg):
    path, filename = image_path_jpeg
    response = client.get(
        f"/image/upload_test_jpeg/{filename}/normalized-tile/zoom/1/ti/0",
    ...
    assert width_resp in (256, width)
    assert height_resp in (256, height)
